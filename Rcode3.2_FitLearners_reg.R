...
corpus <- corpus(string, 
                 docvars=data) #docvars: a data.frame of document-level variables associated with each text
doc.features <- dfm(corpus, tolower=TRUE, ngrams = 1:2, remove =stopwordslist, stem=TRUE, remove_punct = TRUE) 
doc.features <- dfm_trim(doc.features, min_termfreq =2000,  termfreq_type = "rank") 
#save(doc.features, file="doc.features_2gram_2000.Rdata")
...
#save(results, file="results_reg_weight.Rdata")
